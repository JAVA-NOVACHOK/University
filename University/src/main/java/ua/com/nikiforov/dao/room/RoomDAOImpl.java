...
public class RoomDAOImpl implements RoomDAO {
    ...
    public boolean addRoom(int roomNumber) {
        LOGGER.debug("Adding Room with number '{}'", roomNumber);
        boolean actionResult = jdbcTemplate.update(ADD_ROOM, roomNumber) > 0;
        try {
            if (actionResult) {
    ...
    public boolean updateRoom(int number, int id) {
        ...
        try {
            actionResult = jdbcTemplate.update(UPDATE_ROOM, number, id) > 0;
            if (actionResult) {
                LOGGER.info("Successfully updated room with id = '{}', number = '{}'", id, number);
            } else {
                String failMessage = String.format("Couldn't update Room with id - %d number - %s",id, number);
                throw new DataOperationException(failMessage);
            }
        } catch (DataAccessException e) {
            String failMessage = String.format("Couldn't update Room with id - %d number - %d",id, number);
            LOGGER.error(failMessage);
            throw new DataOperationException(failMessage, e);
        }
        return actionResult;
