...
public class RoomsController {
    ...
    public String addRoom(@RequestParam int roomNumber, @RequestParam int seatNumber, Model model) {
        try {
            ...
            model.addAttribute(SUCCESS_MSG, String.format("Room number '%d' with seats number '%d' added successfuly", roomNumber,seatNumber));
    ...
    public String deleteRoom(@RequestParam int roomId, Model model) {
        ...
        actionResult = roomService.deleteRoomById(roomId);
        if(actionResult) {
            model.addAttribute(SUCCESS_MSG, String.format("Room with '%d' deleted successfuly", room.getRoomNumber()));
        }else {
            model.addAttribute(FAIL_MSG, String.format("Failed to update room with number %d and seets number %d", room.getRoomNumber(),room.getSeatNumber()));
        }
        model.addAttribute(ROOM_ATTR,roomService.getAllRooms());
        return VIEW_ROOMS;
    }
    ...
    public String processEdit(@ModelAttribute (MODEL_ATTR_ROOM) Room room, Model model) {
        boolean actionResult = false;
        actionResult = roomService.updateRoom(room.getRoomNumber(), room.getSeatNumber(), room.getId());
        if(actionResult) {
            model.addAttribute(SUCCESS_MSG, String.format("Room number '%d' updated successfuly", room.getRoomNumber()));
        }else {
            model.addAttribute(FAIL_MSG, String.format("Failed to update room with number %d and seets number %d", room.getRoomNumber(),room.getSeatNumber()));
        }
        model.addAttribute(ROOM_ATTR,roomService.getAllRooms());
